...
def test_normal(fit, alpha=0.05, plot=True, palette=None, width=1280, height=640):
    ...
    # '기대 비율 ±2'(SE=√(p(1-p)/n)) 허용 범위 안에 드는지 확인한다.
    ...
    mse_rule = bool(all(mse_pass))                      # 새 구간 모두 충족해야 규칙상 정규성 부합
    ...
    if plot:
        ...
        band_colors = (sb.color_palette(palette, n_colors=3) if palette else ['#0B3C5D', '#328CC1', '#D9EAF7'])
...
def report_fitness(fit, log_y=False, log_x=None, log1p_y=False, log1p_x=None,
    ...
    # --- 1) 변수 라벨 구성 ---
    # log_x, log11p_x는 정확한 독립변수 이름 리스트로 전달된다고 가정한다.
    log_x = log_x or []
    log1p_x = log1p_x or []
    ...
    template = (
        ...
        "{Y}를 종속변수로, {X}(을)를 독립변수로 한 {type}회귀분석 결과,"
        "모형은 통계적으로 {result}.\n\n"
...
def report_variables(fit, data, hc3=False):
    ...
    # 표준오차가 로버스트로 바뀌면 t도 함께 바뀐다.(t = B / 로버스트 SE). 유의확률도 이 로버스트 t에서 나온다. 
    # 따라서 로버스트 SE·t·유의확률을 한 세트로 가져온다. 
    # 로버스트 결과 객체 역시 이름없는 배열로 반환되므로 동일하게 위치 인덱스로 접근한다.
    if hc3:
    ...
    for x in xnames:
        ...
        if hc3:
            ...
            # 각 방식의 SE·t·유의확률이 한 세트로 서로 대응된다.
            row = {
                "종속변수" : yname,                 # 종속변수 이름
                "독립변수" : x,                     # 독립변수 이름
                "B" : b,                           # 비표준화 회귀계수(양쪽 동일)
    ...
    # β의 절대값으로 내림차순 정렬 후 리턴 (영향력이 큰 변수가 위로 오도록)
...
def report_variables_text(fit, log_y=False, log_x=None, log1p_y=False, log1p_x=None, hc3=False,
    ...
    y_pct = y_kind != "none"      # log 계열이면 비율(%) 해석 대상이다.
    ...
    # 독립변수의 변호나 종류를 하나의 값으로 판별한다. (종속변수와 같은 순서로 확인한다)
    def kind_of(name):
  ...
  # --- 3) 그 밖의 정보 (자유도·로버스트 통계량) ---
    ...
    # hc3=True인 경우 로버스트 표준오차 기반 t·유의확률로 교체 ---
    # 회귀계수(B)는 그대로이고, 표준오차만 이 분산에 강건한 HC3로 바뀐다.
    ...
    for x in xnames:
        ...
        # 이 독립변수의 변화나 여부 (정확한 이름 리스트라고 가정)
    ...
    # --- 6) 로버스트 표준오차·log1p 사용 시 해석 주의 각주 첨부 ---
    report = "\n".join(lines)
    ...
    if uses_log1p:
        report += (
            "\n\n> ※ **log1p**(=ln(1+e·))의 % 해석은 변수 자체가 아니라 **(1+변수)** 기준이며, "
...
def plot_beta(fit, data, palette=None, title=None, xlabel=None,ylabel=None,
    ...
    # --- 1) 시각화용 데이터 전처리 ---
    # 회귀계수 표 리턴받기 - 배타값 자체는 hc3 여부와 무관하므로 hc3=False로 호출한다.
    ...
    # --- 4) 막대 끝에 β 값 표기 ---
    # 양수 막대는 오른쪽 끝의 바깥쪽(ha="left"), 음수 막대는 왼쪽 끝의 바깥족(ha="right")에 붙도록
    # 정렬 기준을 뒤집고, 막대와 겹치지 않게 부호 방향으로 살짝 띄운다.
    for i in rdf.index:
        beta = rdf.loc[i, "β"]
