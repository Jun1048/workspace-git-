...
def test_1sample(data, column, popmean=0, alpha=0.05):
    ...
    # test_assumption로 정규성 검정 (단일 컬럼이라 등분산성은 수행되지 않음)
    ...
    # 대립가설 방향별 가설을 부등식으로 표현 (H0: 귀무가설, H1: 대립가설, μ:모 평균)
    hypotheses = {
...
def test_paired(data, before, after, alpha=0.05,
                plot=True, palette=None, title=None, 
                xlabel=None, ylabel=None,
                width=1280, height=640, save_path=None):
    
    
    # 같은 행 끼리 짝지어야 하므로 두 컬럼을 함께 결측 행 제거
    ...
    # 양측, 좌측 단측, 우측 단측을 일괄 검정 (항상 after, before 순)
    for alt in ('two-sided', 'less', 'greater') :
...
def test_independent(data, group1, group2, alpha=0.05, plot=True, palette=None,
    ...
    # 두 집단의 컬러명을 수준(level)으로 사용
    lv = [group1, group2]


    # 각 집단 컬럼을 분리하고 결측 제거 (독립표본으로 컬럼별로 따로 제거)
    ...
    if plot:
        ...
        if test_name == 'Mann-Whitney U test':          # 가설 검정 알고리즘 종류(적용된 검정에 맞춰 지정)
...
def posthoc_oneway(data, y, between, alpha=0.05, plot=True, palette=None,
    ...
    if plot:
        ...
        annotator = Annotator(data=df, x=between, y=y,          # 데이터프레임, x축, y축 변수 저장
...
def anova_twoway(data, y, between, alpha=0.05, order=None, plot=True, palette=None,
    ...
    if equal_var:
        ...
        # np = SS_effect / (SS_effect + SS_residual)
    ...
    if plot:
        ...
        for i in range(2):
            ...
            # 각 조합의 평균(점)과 표준오차(오차 막대)를 선으로 이어 평균 프로파일을 그린다.
...
def posthoc_twoway(data, y, between, alpha=0.05, plot=True, summary=True,
    ...
    if summary:
        ...
        # 앞 부분만 떼어내면 각 비교쌍이 어느 첫 번째 요인 수준에 속하는지 알 수 있다.
        ...
        print("평균 차이가 가장 큰 쌍: ({0}) vs ({1})"
              "→ 차이 {2:.3f}, 효과크기 {3}, p-vlaue {4:.3f}".format(
                  big['A'], big['B'], abs(big['diff']), big['effect_size'], big['pval']))
        

        sm = result.loc[result['diff'].abs().idxmax()]
        ...
        for lv in df[between[0]].unique():
            ...
            print("|n[{0}] {1}쌍 중 유의한 쌍: {2}개".format(lv, len(sub), len(sig)))
...
def correlation(data, x, y, alpha=0.05, plot=True, palette=None,
    ...
    # (단순히 이상치가 존재하는것과 상관계수를 왜곡하는 것은 다르다.)
    trimmed = pair.copy()
    for col in (x, y):
        # 울타리는 항상 원본(pari) 기준으로 계산
...
def multi_correlation(data, columns=None, alpha=0.05, plot=True, palette=None,
    ...
    # 각 변수 조합의 상관계수(coef)를 대칭이 되도록 양족에 채운다.
...
def chi2_goodness_of_fit(data, column, expected=None, order=None, alpha=0.05,
    ...
    # --- 1) 관측 빈도 집계 ---
    # 범주별 관측 빈도 집걔 (order가 있으면 그 순서로, 없으면 라벨 순으로 정렬)
    ...
    # --- 2) 기대 빈도 결정 ---
    # None이면 균등 분포, 합이 1 이하인 비율이면 빈도로 환산, 그 외는 빈도로 사용
    if expected is None:
        exp = np.full(k, n / k)
    ...
    # --- 3) 가정 확인 ---
    # 기대 빈도 점검 (5 이상 셀이 80% 이상 + 1 미만 셀 없음)
    ...
    # --- 7) 결과 시각화 ---
    # 관측 빈도 vs 기대 빈도 막대 그래프 (long 형식으로 변환해 hue로 비교)
    if plot :
...
def _chi2_crosstab(data, row, col, kind, alpha=0.05, plot=True, palette=None, orient='v',
    ...
    # --- 1) 대상 데이터 전처리 ---
    # 두 범주형 변수로 교차료(관측 빈도) 생성
    ct = crosstab(data[row], data[col])



    # --- 2) 가정 확인 ---
    # 카이제곱으로 기대 빈도 확보 (2x2는 예이츠 보정 기본 적용) 후 가정 점검
    ...
    # --- 3) 검정 수행 (가정에 따른 분기) ---
    # 가정 위반 + 2x2이면 피셔의 정확검정으로, 그 외는 카이제곱으로 분기
    ...
    # --- 4) 강도 확인 ---
    # 효과 크기 : 크라메르 V(피셔로 분기해도 chi2 기반 참고치를 함께 제공)
    n = int(ct.values.sum())
    min_dim = min(ct.shape) - 1
    cramers_v = float(np.sqrt(chi2 / (n * min_dim))) if min_dim > 0 else np.nan

    # 효가 크기 강도 라벨 (Cohen 관례 :  0.1↓ 미미 / 0.3↓ 약함 / 0.5↓ 보통 / 그 이상 강함)
    if cramers_v < 0.1:
        strength = 'Negligible'
...
def chi2_independence(data, x, y, alpha=0.05,
    ...
    return _chi2_crosstab(data, x, y, 'independence', alpha, plot=plot,
                          palette=palette, orient=orient, title=title,
...
def chi2_homogeneity(data, group, category, alpha=0.05,
    ...
    return _chi2_crosstab(data, group, category, 'homogeneity', alpha, plot=plot,
                          palette=palette, orient=orient, title=title,
                          xlabel=xlabel, ylabel=ylabel,
                          width=width, height=height, save_path=save_path)
...
def correlation_summary(corr_df, strength='Strong'):
    ...
    corr_table.rename(columns={'join':'collumns'}, inplace=True)
