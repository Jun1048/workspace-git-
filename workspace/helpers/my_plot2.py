...
def lineplot(data=None, 
             ...
             color=None, linewidth=2.0, linestyle='-', palette=None, marker=None, markersize=None, markeredgewidth=None, markeredgecolor=None, markerfacecolor=None,
    ...
    # 선 그래프 그리기
    sb.lineplot(data=data, x=x, y=y, hue=hue, color=color, linewidth=linewidth, linestyle=linestyle,
                palette=palette, marker=marker, markersize=markersize, markeredgewidth=markeredgewidth, markeredgecolor=markeredgecolor, markerfacecolor=markerfacecolor, ax = ax, **params)
...
#---------------------------------------------------------------
# 단변량 커널밀도 그래프 모듈화
def kdeplot(data=None, 
            x=None, hue=None, meanline=False, clevel=0,
    ...
    if meanline:
        ...
        if hue is None:
            ...
            ax.text(x=mv + 0.05, y=y_max*0.95, s=f'Mean: {mv:.2f}', color='red', fontsize=14, ha='center')

        else : 
            # hue 범주별 평균선 표시 (kdeplot이 그린 라인의 색상과 일치)
            categories = list(data[hue].unique())
            # 팔레트에서 범주의 수에 맞는 색상값 추출
            colors = sb.color_palette(palette, n_colors=len(categories))
            ...
            for i, cat in enumerate(categories):
                ...
                ax.text(x = mv + 0.05, y= y_max *(0.95 - i*0.07), s=f'{cat} Mean: {mv:.2f}', color=colors[i], fontsize=14, ha='center')
...
def histplot(data=None, x=None, bins='auto', 
    ...
    # 히스토그램 그리기
    sb.histplot(data=data, x=x, hue=hue, linewidth=linewidth, palette=palette, bins=bins, ax=ax, kde=kde)
...
def boxplot(data=None,
             x=None, y=None, 
             hue=None, orient=None, palette=None, order=None, 
             title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None, ax=None):
    ...
    # 범주축과 hue가 겹치는 경우 상자가 쪼개지지(dodge) 않도록 처리
    hue, dodge, legend = _resolve_hue(x if orient != 'h' else y, hue=hue, palette=palette)

    # 상자그림 그리기
    sb.boxplot(data=data, x=x, y=y, hue=hue, orient=orient, order=order, dodge=dodge, legend=legend, palette=palette, ax=ax)
...
#---------------------------------------------------------------
# 바이올린 플롯 모듈화
def violinplot(data=None,
             x=None, y=None, 
             hue=None, orient=None, palette=None,
             title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None, ax=None):
    ...
    # 바이올린 플롯 그리기
    sb.violinplot(data=data, x=x, y=y, hue=hue, orient=orient, palette=palette, dodge=dodge, legend=legend, ax=ax)
...
def heatmap(data=None, annot=True, fmt='0.2f', linewidth=0.5,
            palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None, ax=None, **params):
...
#---------------------------------------------------------------
# 막대그래프 (Barplot)
def barplot(data=None, x=None, y=None, 
            hue=None, palette=None, estimator=np.mean, order=None, 
            title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None, ax=None, **params):
    ...
    Args : 
        ...
        x : x축 범주 컬럼명
        y : y축 값 컬럼명
        estimator : 막대 높이 계산함수 ( 기본값 : np.mean)
    ...
    # 막대그래프 그리기
    sb.barplot(data=data, x=x, y=y, hue=hue, palette=palette, estimator=estimator, order=order, dodge=dodge, legend=legend, ax=ax, **params)
...
#---------------------------------------------------------------
# count그래프
def countplot(data=None, x=None, y=None, 
            hue=None, palette=None, order=None, 
            title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None, ax=None, **params):
    ...
    # 카운트그래프 그리기
    sb.countplot(data=data, x=x, y=y, hue=hue, palette=palette, order=order, dodge=dodge, legend=legend, ax=ax, **params)
...
def pieplot(x, labels, autopct='%0.1f%%', startangle=90, counterclock=False,
    ...
    if donutchart:
        wedgeprops = {'width' : wedge_width, 'edgecolor': wedge_color,
                      'linewidth' : wedge_width}
    
    #파이 그래프 그리기
    ax.pie(x, labels=labels, autopct=autopct, startangle=startangle,
           counterclock=counterclock, explode=explode, colors=color_list, wedgeprops=wedgeprops, **params)
...
def stackplot(data, x, y, hue, aggfunc=np.sum, orient='v', ratio = False,
               text=True, text_color='#ffffff', text_fontsize=12, text_format='{:.1f}', palette=None,
               title=None, xlabel=None, ylabel=None,
               width=1280, height=640, save_path=None, ax=None, **params):
    ...
    for i, col in enumerate(df.columns):
        ...
        if text:
            for j, val in enumerate(df[col]):
                if val == 0 : #누적값이 0인 경우 텍스트를 표시하지않음
                    continue
                
                if orient =='v':
                    ax.text(x=j, y=df.iloc[j, :i].sum() + val / 2, 
                            s=text_format.format(val), ha='center', va='center', color=text_color, fontsize=text_fontsize)
...
def lmplot(data, x, y, hue=None, palette=None, col=None, row=None, markers='o',
           ...
           scatter_alpha=0.7, linestyle='-', linecolor=None,  linewidth=2,
           title=None, xlabel=None, ylabel=None,  width=1280, height=640, save_path=None, ax=None):
    ...
    # hue 가 지정되지 않았는데, palette와 linecolor가 지정된 경우, 무의미하므로 None로 설정
    ...
    g = sb.lmplot(data=data, x=x, y=y, height=h, aspect=w/h, hue=hue, col=col, row=row, legend=False, markers=markers, palette=palette,
                  scatter_kws={
                      'edgecolor' : scatter_edgecolor, 'linewidths' : scatter_linewidths, 's' : scatter_size, 'alpha' : scatter_alpha
                  }, 
...
def pointplot(data=None, x=None, y=None, hue=None, order=None, hue_order=None, 
    ...
    '''
    점 그래프 (pointplot)를 그린다.
    범주별 추정치(기본 : 평균)와 오차범위를 점과 선으로 표현하므로 
...
#-----------------
# hue 해결 모듈 정의
def _resolve_hue(cat_axis, hue=None, palette=None):
    '''
    범주형 그래프 (boxplot, violinplot, barplot, countplot)의 hue 관련 인자를 결정한다.

    범주축을 기준으로 다음 두 경우를 처리한다.
        - hue 없이 palette만 지정 : 범주축을 hue 로 삼아 범주별 색상을 적용
        - hue 가 범주축과 동일 (중복) : 외부에서 hue=x 로 호출된 경우
    두 경우 모두 hue 가 범주축과 같으므로, 그래프가 쪼개지지(dodge) 않게 끄고 범례도 숨긴다.
    
    Args : 
        cat_axis : 범주축 컬럼명 (세로형이면 x, 가로형이면 y)
        hue : 범주 구분 컬럼명
        palette : 색상 팔레트 이름
    
    Returns : 
        hue : seaborn에 전달할 hue 값
        dodge : seaborn에 전달할 dodge 값
        legend : seaborn에 전달할 legend 값
    '''

    if hue is None and palette is not None:
        return cat_axis, False, False
    
    if hue is not None and hue == cat_axis:
        return hue, False, False
    
    return hue, 'auto', 'auto'