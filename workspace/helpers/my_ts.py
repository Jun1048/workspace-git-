...
# -------------------------------------------------
# 시계열 데이터를 구간으로 나누어 구간별 통계를 내는 함수수를 비교한다.
# -------------------------------------------------
def report_split(data, column=None, size=3):

    """ 시계열을 여러 구간으로 잘라 구간별 평균·표준편차·변동계
...
def transform(data, column=None, log=False, diff=0, seasonal_diff=0, period=None):
    ...
    Args:
        ...
        period (int): 계절 주기. seasonal_dioff가 1 이상이면 반드시 지정 (기본값: None).
...
def adf_diff(data, column=None, max_diff=-1, alpha=0.05):
    ...
    while True:
        ...
        if max_diff < 0:                                       # 최대 차분 횟수를 지정하지 않았다면
            ...
            if previous_std is not None and current_std > previous_std:
                stopped = name                                 # --- 과대 차분이 시작된 차수 이름 기록
                break                                          # --- 반복을 멈춘다
    ...
    # --- 2) 표준편차가 최소인 차수 확인 --> 과대 차분 시작 직전 지점 ---
    ...
    result__df["권장"] = recommend                               # 권장 표시를 결과표에 추가
    ...
    # 남은 원인은 대개 계절성고 분산이며, 그것은 차분 횟수로 풀 문제가 아니다.
    if not result_df.loc[best, "정상성"]:
        pvalue = result_df.iloc[best, "p-value"]
        print(f"※ {best}은 아직 비정상입니다(p-value {pvalue}). 차분을 더 늘리지 말고"
...
def adf_transform(data, column=None, period=None, max_diff=2, log=True, alpha=0.05):
    ...
    Args:
        ...
        max_dff (int): 조합에 포함할 최대 일반차분 횟수 (기본값: 2)
    ...
    for use_log in log_options:                         # 로그변환 여부 후보 순회
        for diff in range(max_diff + 1):                # 일반차분 횟수 후보 순회
            for seasonal_diff in seasonal_options:      # 계절차분 여부 후보 순회
                ...
                if len(parts) == 0:                     # 조합이름이 비어있다며 원본임
                    name = "원본"
    ...
    if len(smaller) > 0:
        ...
        print(f"※ {name}의 표준편차({std})가 더 작습니다. "
...
def plot_rolling(data, window, column=None, title=None, xlabel=None, ylabel=None,
    ...
    """ 원본과 이동 평균 · 이동 표준편차를 한 그래프에 그려 정상성을 눈으로 확인한다.

    정상 시계열이라면 이동 평균과 이동 표준편차가 모두 수평선에 가까워야 한다.


    Args:
        data (DataFrame | Series): 날짜 인덱스를 가진 시계열 데이터.
        window (init): 이동 계산에 사용할 창 크기. 계절 주기로 잡는다.
        column (str): data가 데이터프레임인 경우 대상 컬럼명 (기본값: None)
    ...
    result_df = DataFrame({
        "원본": data,
        "이동 평균": data.rolling(window).mean(),
        "이동 표준편차": data.rolling(window).std(),
    })
    ...
    # --- 3) 그래프 그리기 및 결과 반환 ---
    if title is None:
        title = f"원본 · 이동 평균 · 이동 표준편차 (window={window})"
...
def compare_smoothing(data, sizes, column=None, method="ma", overlay=False,
    ...
    Args:
        data (DataFrame | Series): 날짜 인덱스를 가진 시계열 데이터.
        size (list | tuple): 비교할 창 크기(또는 span) 목록.
        column (str): data가 데이터프레임인 경우 대상 컬럼명 (기본값: None)
        method (str): "ma"(이동 평균) · "ewm"(지수 평활) · "both"(둘 다) (기본값: "ma")
    ...
    for size in sizes:                          # 각 창 크기별로 평활 계산
        ...
        if method == "ma" or method == "both":
            result_df[f"이동 평균({size})"] = data.rolling(size).mean()
        ...
        if method == "ewm" or method == "both":
            ...
            result_df[f"지수 평활({size})"] = data.ewm(span=size).mean()


    # 그래프를 그리지 않으면 결과표만 반환하면 여기서 중단
    if not plot:
        return result_df
...
def decompose(data, period, column=None, model=None, plot=True, title=None,
    ...
    # 주기 단위로 묶어, 표준편차보다 변동계수보다 더 일정하면 승법으로 본다.
    if model is None:                        
...
def report_seasonal(data, period, column=None, model=None, plot=True,
    ...
    # --- 3) 기준 대비 크기 ---
    # 승법이 1이 기준이므로 백분율로, 가법은 0이 기준이므로 절대값으로 읽는다
...
def auto_correlation(data, column=None, period=None, plot=True,
    ...
    result_df["q"] = np.where(result_df.index == p, "◀", "")
    ...
    for name, code, order, kind, step in (("AR", "p", p, "PACF", 1), 
        ...
        print(f"{name} 차수 후보 ({code}): {order}  ← {reason}")                    # p, q에 대한 판정 결과
    ...
    print(f"→ SARIMA({p}, d, {q})({P}, D, {Q})[{period}]"
          f"- d·D 는 차분 단계의 결과를 넣는다")                    # 계절성 → P, Q에 대한 판정 결과            
    ...
    # --- 10) 가장 뒤쪽 차수의 lag가지만 반환 ---
    # 차수가 하나도 잡히지 않았다면 자를 기준이 없으므로 근거를 통재로 돌려준다.
...
def fit_model(data, column=None, p=1, d=1, q=1, P=1, D=1, Q=1, s=None,
    ...
    # 로그변환은 여기서 한번만 한다. 차분은 SARIMAX가 d·D로 알아서 처리한다.
...
def predict(fit, steps, alpha=0.05, plot=True, history=None,
    ...
    Args:
        ...
        steps (int): 내다볼 시점의 개수. 계절 주기를 넣으면 한 시즌이 된다ㅣ.
...
def report_residual(fit, alpha=0.05, report=True, plot=True, title=None,
    ...
    # --- 3) 무상관 : Ljung-Box 검정 ---
    # 여러 시차를 한 묶음으로 검정해 "남은 자기 상관이 있는가"를 판정한다
    # 계절 주기가 있으면 한 시·두 시즌, 없으면 관례값인 10 시차를 본다
    # (Hundman & Athanasopoulos, 2021), statsmodels 내부 코드도 10 시차를 기본값으로 쓴다
    ...
    # 시차가 하나도 남지 않으면 관례값을 쓰되, 관측치 수의 1/5 보다 크면 안된다.
...
def report_score(fit, name = "적합값", report=True):
    ...
    # --- 3) 기준선(나이브) 준비 ---
    # 계절주기가 있으면 작년 같은 달, 없으면 직전 시점을 그대로 예측값으로 쓴다
    ...
    nonzero = y != 0                           # 관측값이 0인 시점은 MAPE를 계산할 수 있다.
    result = []                                # 지표를 담을 리스트
    ...
    for target in table.columns[1:]:           # 0번째 컬럼은 관측값이므로 제외
        ...
        result.append({
            ...
            "MAPE(%)": round(np.mean(np.abs(error[nonzero] / y[nonzero])) * 100, 3) if nonzero.any() else np.nan,
        })
