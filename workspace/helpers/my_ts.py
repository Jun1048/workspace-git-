...
def adf_transform(data, column=None, period=None, max_diff=2, log=True, alpha=0.05):
    ...
    result = []                             # 결과를 담을 리스트
    changed = {}                            # ⚠️ 누락된 변수 추가: 변환된 데이터를 저장할 딕셔너리
    ...
    for use_log in log_options:                         # 로그변환 여부 후보 순회
        for diff in range(max_diff + 1):                # 일반차분 횟수 후보 순회
            for seasonal_diff in seasonal_options:      # 계절차분 여부 후보 순회
                ...
                # ⚠️ 누락된 로직 추가: 결측치 제거 후 딕셔너리에 저장
                clean_target = target.dropna()
                changed[name] = clean_target

                # 결측치 제거 후 ADF 검정
                result.append(adf_test(clean_target, name=name, alpha=alpha))
...
#---------------------------------------------------
# 분해 시계열 함수
def decompose(data, period, column=None, model=None, plot=True, title=None, rows=2, cols=2, width=960, height=480, save_path=None):
    '''
...
def report_seasonal(data, period, column=None, model=None, plot=True, 
                    title=None, xlabel=None, ylabel=None, width=1000, height=400, save_path=None):
    '''
    계절 성분을 주기 안의 위치별로 정리해 계절지수표를 만든다.
    승법 모델의 계절지수는 배수로 읽는다. 1.227은 추세선보다 22.7% 높다는 뜻!
    
    Args : 
        data (DataFrame | Series) : 날짜 인덱스를 가진 시계열 데이터
        period (int) : 계절 주기
        column (str) : data가 데이터프레임인 경우 대상 컬럼명 (기본값 : None)
        model (str) : 'additive' 또는 'multiplicative', None이면 자동판정 (기본값 : None)
    ...
    if model is None:                                               # 사용할 모델이 지정되지 않았다면?
        block = np. arange( len(data) ) // period                   # 주기 단위로 묶어 블록 번호를 만든
        block_std = data.groupby(block).std()                      # 각 블록별 표준편차
        block_mean = data.groupby(block).mean()                    # 각 블록별 평균
        block_cv = block_std / block_mean                           # 각 블록별 변동계수
...
def adjust_seasonal(data, period, column=None, model=None, plot=True, title=None,
    ...
    if plot:
        ...
        my_plot.lineplot(x=data.index, y=data, label="", ax=ax)
...
title=None, save_path=None):
    ...
    if period is not None:
        for name, code, order, kind, step in (("계절 AR", "P", P, "계절 PACF", period), ("계절 MA", "Q", Q, "계절 ACF", period)):
            reason = (f"{kind}가 lag {order * step} 이후 절단" if order else f"{kind}가 lag {step}부터 비유의")
            print(f"{name} 차수 후보 ({code}): {order} ← {reason} (판정 시차 {season_lags})")
        
    print(f"- SARIMA({p}, d, {q})({P}, D, {Q})[{period}] - d·D 는 차분 단계의 결과를 넣는다")
    ...
    if plot:
        ...
        if period is not None and s_order:
            ...
            subtitle += f" | 계졀 lag {s_order * period}까지 유의 → {s_code} = {s_order}"
...
def fit_model(data, column=None, p=1, d=1, q=1, P=1, D=1, Q=1, s=None, 
    ...
    Args : 
        ...
        report (bool) : 조합별 AIC·BIC 결과표 출력 여부 (기본값 : False)
    ...
    for try_p, try_d, try_q, try_P, try_D, try_Q in orders:
        ...
        # --- 3-3) 최적 모형 갱신 ---
        # AIC가 더 낮은 모형을 만나면 그 자리에서 갈아끼운다. (다시 적합할 필요가 없다)
...
def predict(fit, steps, alpha=0.05, plot=True, history=None,
            title=None, xlabel=None, ylabel=None, width=1280, height=520, save_path=None):
    '''
    ...
    Returns : 
        DataFrame : 미래 시점을 인덱스로 하는 예측표 (pred · lower · upper)
    '''
