...
def pca(df,y=None, n_components=0.8, scale=True, method='standard',
    ...
    Args:
        ...
        hue_palette (str) : 바이플롯에서 종속변수를 구분할 색상 팔레트 (기본값: 'tabl10')
    ...
    Returns:
        DataFrame: 주성분 점수(PC1, PC2, ...) 데이터 프레임. y를 지정했다면 종속변수가 함께 붙는다.
    ...
    # --- 1-1) 종속변수 분리 ---
    # 차원축소는 독립변수에만 적용하는 기법이므로 종속변수는 미리 데어 둔다.
    if y is not None:
        target = df[y]
