...
from sklearn.preprocessing import StandardScaler, MinMaxScaler, RobustScaler, robust_scale, MaxAbsScaler
...
def long2wide(df, hue, values, dropna=True):
    ...
    wide = pivot_table(data=df, index=df.groupby(hue).cumcount(), 
...
def reduce_vif(df, columns=None, threshold=10.0, verbose=True):
    ...
    Args : 
        ...
        columns (list, optional) : 다중 공선성을 판단/제거할 컬럼명 리스트
            None 이면 df의 수치형 컬럼을 자동으로 선택한다. (기본값 : None)
        threshold (float) : 다중 공선성 판단 기준이 되는 VIF 임계값 (기본값 : 10.0)
        verbose (bool) : 제거 과정과 결과를 단계별로 출력할지 여부 (기본값 : True)

    Returns :
        DataFrame : 대상 변수들의 VIF가 모두 threshold 미만이 되도록 일부 변수가 제거된 데이터 프레임,
        대상이 아닌 컬럼은 원래 순서대로 보존된다.
    ...
    # 대상에서 제외되는 컬럼(종속변수, 명복형 등)은 그대로 보존하기 위해 컬럼이름을 따로 기록
    keep = []
    for c in df.columns:
        if c not in targets:
            keep.append(c)
...
def pca(df, y=None, n_components=0.8, scale=True, method='standard',
    ...
    if plot:
        ...
        if isinstance(n_components, float) and 0 < n_components < 1:
            ...
            ax[0].text(1, n_components + 0.01, f'(n_components={n_components})', color='red')
