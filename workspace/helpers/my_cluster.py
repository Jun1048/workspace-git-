...
# 덴드로그램(계층적 군집이 합쳐온 나무 모양으로 그려주는 함수)
...
# --------------------------------------
# KMeans 군집화 함수 정의 및 작업 단계 구성
# --------------------------------------
def kmeans(data, k, columns=None, scaling='standard', cluster_name='그룹번호',
           random_state=RANDOM_STATE, verbose=True, plot=True, x=None, y=None,
           title=None, palette='tab10', size=100, edgecolor='#ffffff', linewidth=1.5,
           alpha = 1, outline=True, center=True, center_marker='X', center_size=150,
           center_color='#ff0000', center_edgecolor='#000000', center_linewidth=1.5,
           equal_scale=False, width=1280, height=640, save_path=None, ax=None, ):  # n_init=10 을 추가가능(컴터 성능이 좋을 때)


    """ 데이터를 k개의 군집으로 나누고, 군집 결과와 중심점을 시각화 하는 함수
    ...
    Args (기본값은 위의 함수 정의 참고):
        ...
        vaerbose: 스케일링 전후의 값의 범위를 출력할지 여부
    ...
    # n_init: 시작 위치를 여러번 바꿔 시도한 뒤 가장 좋은 결과를 채택한다.
    estimator = KMeans(n_clusters=k, random_state=random_state)          # n_init=n_init 추가 가능(함수 파라미터에 n_init=10 이 추가되었을 때)
    estimator.fit(df)
...
def best_k_elbow(data, klist=None, columns=None, scaling='standard',
    ...
    Args (기본값은 위의 함수 정의 참고):
        ...
        n_init: 시작 위치를 바꿔가며 시도할 횟수 (k끼리 공정하게 비교하려면 1회로는 부족하다)
        sensitivity: KneeLocator의 민감도(S). 작을수록 작은 꺾임에도 반응한다.
        verbose, plot, title: 계산 결과 출력 여부, 시각화 여부, 그래프 제목(None이면 자동 생성)
        color, marker, linestyle, best_color: 이너셔 선의 색상·마커 모양·선 스타일, 엘보우 포인트를 표시할 세로선의 색상
        width, height, save_path, ax: 캔버스 가로·세로 픽셀, 저장 경로, 그래프를 그릴 Axes 객체(None이면 새로 생성)
...
# ---------------------------------------------
# 실루엣 스코어와 면적 두께를 함께 평가해 최적 k 찾기
# ---------------------------------------------
...
def best_k(data, klist=None, columns=None, scaling='standard',
    ...
    # --- 4) 최종 판단 ---
    final_k = sil_k                     # 엘보우는 뭉침만 보므로 후보 재사용, 최종 결정은 분리와 균형까지 보는 실루엣이 맡는다.
...
def plot_dendrogram(estimator, cut_height=None, title=None,
    ...
    Args (기본값은 위의 함수 정의 참고):
        estimator: compute_distances=True 로 학습한 AgglomerativeClustering 모델
        cut_height: 나무를 자른 높이(병합 거리). 이 높이 아래의 가지를 군집별로 다른 색으로 칠하고 가로선으로 표시한다.
                    None이면 모델이 실제로 자른 높이를 직접 계산해서 쓰므로, 보통은 지정할 필요가 없다
        title, xlbael, ylabel: 그래프 제목은(None이면 '덴드로그램'), 
                               x축, y축 이름(xlbael이 None이면 가지를 묶어 그리는지에 따라 자동으로 정한다)
        p, truncate_mode: 표시할 가지의 개수와 생략 방식('lastp' = 마지막 q개의 가지만, None이면 데이터 전체를 그린다)
        leaf_rotation, leaf_font_size, count_sort: 아래쪽 누금의 회전 각도, 글자 크기, 
    ...
    # --- 1) 모델이 합쳐온 과정을 dendrogram()이 읽는 표시(linkage 행렬)로 변환 ---
