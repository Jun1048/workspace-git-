...
from IPython.display import display, Markdown  # 결과 출력을 위한 함수
...
from . import my_prep
from . import my_plot
from . import my_stats  # VIF 계산을 위한 함수
...
def fit_model(data, y, summary=False):
    ...
    Args:
        ...
        y: 종속변수로 사용할 컬럼명. `data`에 반드시 존재해야 하며 0/1의
        이분형이어야 한다.
...
def plot_sigmoid(fit, data, x, threshold=0.5, palette=None, title=None,
    ...
    """
    # --- 1) 그릴 종속변수 결정 ---
    yname = fit.model.endog_names  # 종속변수 이름은 모델 객체에서 추출 가능
    ...
    # --- 3) 그래프 초기화 ---
    # 팔레트가 지정되었다면 첫 번째 색상을 선 색상으로 사용하고, 지정되지 않았다면 기본
    # 파랑색을 사용한다
...
def report_variables(fit, data):
    """적합된 로지스틱 모델의 독립변수별 회귀계수·오즈비 보고표를 데이터프레임으로
    생성해 반환한다.
    ...
    # |B|(=|log OR|)의 절댓값으로 내림차순 정렬 후 리턴 (영향력이 큰 변수가 위로 오도록)
...
def report_variables_text(fit, data=None, alpha=0.05):
    ...
    Returns:
        str: 독립변수별 해석 문장 불릿 리스트. `IPython.display.Markdown`으로 감싸
             출력하면 좋다.
    """
    # --- 1) 해석 대상 결정 (상수항 제외) ---
    yname = fit.model.endog_names
    ...
    for x in xnames:
        ...
        if is_binary:
            change = f"**{x}**가 해당하는 경우(기준 범주 대비)"
        else:
            change = f"**{x}**가 1 증가할 때"
...
def plot_odds(fit, data, palette=None, title=None, xlabel=None, ylabel=None,
    ...
    # --- 3) 가로 막대그래프 ---
    my_plot.barplot(rdf, x="오즈비(OR)", y="독립변수", hue="부호",
...
#--------------------------------
# 로지스틱 회귀 모형 적합 + 후진 소거 자동처리 + 결과보고 자동화 함수
...
def test_linear(fit=None, data=None, yname=None, xnames=None, targets=None, alpha=0.05):
    ...
    Args:
        fit: 'fit_model' 함수로 적합된 회귀분석 결과 객체.
        data: 회귀분석에 사용한 원본 데이터프레임. 독립변수와 종속변수를 모두포함해야 한다.
        yname (str) : 종속변수 컬럼명
...
def fix_linear(data, y, alpha=0.05, allow_shifted_log=False, max_rounds=None, report=True):
    ...
    for rnd in range(1, max_rounds + 1):
        ...
        # --- 3-5) 두 처방을 견주어 채택한다 ---
        # 제곱항은 단조 곡선도 근사하므로, '유의하지 않을 때만 로그'로 두면 로그가 맞는 경우에도 제곱항이
        # 먼저 채택된다.
