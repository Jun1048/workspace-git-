...
from pandas import DataFrame, melt, concat, Series, notna, crosstab
...
def ci(data, column=None, clevel=0.95):
    ...
    Args:
        data (Series | list | ndarray | DataFrame) : 연속형 데이터 또는 데이터 프레임
    ...
    # 데이터 프레임 + 컬럼명 형태로 전달된 경우 해당 컬럼만 추출
...
def test_assumptions(data, columns=None, alpha=0.05, center='median'):
    ...
    등분산성 검정을 수행한다. 
    이때 모든 변수가 정규성을 충족하면 Bartlett 검정을,
    ...
    if len(columns) > 1 : 
        # 각 컬럼을 실수형으로 변환하여 리스트로 추출 (Bartlett은 실수형이 필요)
...
def test_1sample(data, column, popmean=0, alpha=0.05):
    ...
    양측 / 좌측단측 / 우측단측 세가지 대립가설을 일괄 검정한다.

    
    Args : 
        data (DataFrame) : 검정 대상 데이터프레임
        column (str) : 검정할 연속형 컬럼명
        popmean (float) : 비교 기준이 되는 모평균 μ0 (기본값 : 0)
...
#----------------------------------------------------------------------------------------
# 대응표본 T 검정 함수 모듈화
def test_paired(data, before, after, alpha=0.05, plot=True, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    '''
    짝지어진 두 측정값 (전 / 후)의 차이가 있는지 검정하는 함수 (wide형식)

    차이값 d = after - before 의 정규성 충족 시 대응표본 t검정, 미충족 시 Wilcoxon 부호 순위 검정을 수행하며,
    ...
    Args : 
        ...
        alpha (float) : 유의수준 (기본값 : 0.05)
        plot (bool) : 결과를 시작화할지 여부 (기본값 : True)
    ...
    if plot : 
        ...
        #독립 표본 T검정 결과를 시각화에 추가
        annotator = Annotator(data=melt_df,                     # 데이터 프레임
                              x='group',                        # x축 변수
                              y='value',                        # y축 변수
...
def test_independent(data, group1, group2, alpha=0.05, plot=True, palette=None,
    ...
    Args : 
        ...
        palette (str or list) : 색상팔레트 (기본값 : None)
        title (str, optional) : 그래프 제목 (기본값 : None)
        xlabel (str, optional) : x축 라벨 (기본값 : None)
        ylabel (str, optional) : y축 라벨 (기본값 : None)
        width (int, optional) : 그래프 가로 크기 (기본값 : 1280)
        height (int, optional) : 그래프 세로 크기 (기본값 : 640)
    ...
    if not both_normal:
        ...
        test_name = 'Studen t-test'                 # 정규성 충족 + 등분산
    else:
    ...
    # 시각화 옵션이 True 인 경우, 시각화 수행
    if plot:
        # wide 형식을 long 형식으로 변환하여 그룹별 박스플롯 각성
...
def anova_oneway(data, y, between, alpha=0.05):
    ...
    # 집단 별 종속변수 값을 wide 형태(집단=컬럼)로 모아 가정 검정에 전달
    ...
    #--- 효과크기 해석 컬럼 추가 ---
    # pingouin이 제공하는 Cohen의 효과 크기 기준표로 해석하여 라벨 부여
...
def posthoc_oneway(data, y, between, alpha=0.05, plot=True, palette=None, summary=True, 
                   title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    ...
    Args :
        data (DataFrame) : 검정 대상 데이터프레임 (long형식)
        y (str) : 종속변수(연속형) 컬럼명
        between (str) : 종속변수(연속형) 컬럼명
        alpha (float) : 유의수준 (기본값 : 0.05)
        plot (bool) : 결과를 시각화할지 여부 (기본값 : True)
        palette (str or list) : 색상 팔레트 (기본값 : True)
    ...
    # 1) 분석에 사용할 두 컬럼만 추출하고 결측행 제거
    df = data[[y, between]].dropna()

    # 2) 집단별 종속변수 값을 wide 형태(집단=컬럼)로 모아 등분산성 가정 검정에 전달
    wide = my_prep.long2wide(df, hue=between, values=y)
    assumption = test_assumptions(wide, columns = list(wide.columns), alpha=alpha)
    ...
    # 어떤 사후검정을 사용했는지 식별할 수 있도록 맨 앞에 test컬럼 추가
    ...
    # Cohen의 d기준표를 따라 절대값으로 해석한다.
    # >= 0.8 - 큼, >= 0.5 - 중간, >= 0.2 -작음, 그 미만 - 미미함
    # 부호는 비교 방향(A-B)을 의미하므로 크기 해석에는 절댓값을 사용한다.
    abs_hedges = result['hedges'].abs()
    conditions = [abs_hedges >= 0.8,
                  abs_hedges >= 0.5,
                  abs_hedges >= 0.2]
    
    labels = ['Large', 'Medium', 'Small']
    result['effect_size'] = np.select(conditions, labels, 'Negligible')
...
#-------------------------------------------------------------------
# 이원분산분석 함수 정의
def anova_twoway(data, y, between, alpha=0.05, order=None, plot=True, 
                 palette=None, title=None, ylabel=None, width=640, height=480, save_path=None):
    ...
    if equal_var:
        ...
        # 등분산 미충족 -> 등분산을 가정하지않는 선형모형(HC3) 기반 Type-II ANOVA
        test_name = 'Type-II ANOVA'
        # Q () 로 컬럼명을 감싸 공백 / 특수문자가 있는 컬럼명도 안전히 처리
        ...
        # statsmodels 결과표에는 np2 가 없으므로 편에타제곱을 직접 계산한다
        ...
        # statsmodels 가 만든 효과명(C(Q('water')) : C(Q('sun')) 등)을
        # pingouin 결과와 동일한 형태 (water * sun)로 정리한다.
        aov['Source'] = (aov['Source'].str.replace("'C(Q('", "", regex=False).str.replace("'))", "", regex=False).str.replace(":", " * ", regex=False))
    ...
    labels = ['Larges', 'Medium', 'Small']
    aov['effect_size'] = np.select(conditions, labels, 'Negligible')
    #np2 가 없는 행(잔차 등)은 효과크기 해석 대상이 아니므로 표시를 비운다.
    ...
    if plot :
        ...
        # (title은 전체 목록(suptitle)이며, 지정하지않으면 서브플롯별 제목만 표시된다.)
        ...
        for i in range(2):
            ...
            my_plot.pointplot(data=df, x=x_field, y=y, hue=hue_field,
                              order=x_order, hue_order=hue_order, errorbar='se', capsize=0.1, palette=palette,
                              legend_title=hue_field, ax=ax[i])
            
            # ax를 직접 전달한 경우 my_plot이 제목과 축 라벨을 설정하지 않으므로 직접 지정한다.
            ax[i].set_title('{0}에 따른 {1}별 평균 {2}'.format(x_field, hue_field, y), fontsize=15, pad=0)
...
#-------------------------------------------------------------------
# 이원분산분석 사후 검정 함수 정의
def posthoc_twoway(data, y, between, alpha=0.05, plot=True, summary=True, palette=None, 
                   title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    '''
    ...
    Args : 
        data (DataFrame) : 검정 대상 데이터프레임(long형식)
        y (str) : 종속변수(연속형) 컬럼명
        between (list) : 집단을 구분하는 두 개의 독립변수(명목형) 컬럼
        alpha (float) : 유의수준 (기본값 : 0.05)
        plot (bool) : 결과를 시각화할지 여부 (기본값 : True)
        summary (bool) : 결과 요약(비교쌍 수, 유의한 쌍 수, 평균차가 가장 큰/작은 쌍, 
                        첫번째 요인의 수준별 유의한 쌍)을 축력할지 여부 (기본값 : True)
        palette (str or list) : 색상 팔레트 (기본값 : None)
    ...
    if equal_var:
        ...
        # (pingouin 버전/패치에 따라 'p-tukey' 또는 'p_tukey'로 나올 수 있음)
        pcol = 'p-tukey' if 'p-tukey' in result.columns else 'p_tukey'
    ...
    # hedges는 두 집단 평균차에 대한 표준화 효과 크기로,
    ...
    if summary:
        ...
        # 앞부분만 떼어내면 각 비교쌍이 어느 첫번째 요인 수준에 속하는지 알 수 있다.
        ...
        # 첫번째 요인의 수준을 고정한 채 두번째 요인끼리만 비교한 쌍을 따로 살핀다.
...
def correlation(data, x, y, alpha=0.05, plot=True, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    ...
    # (단순히 이상치가 존재하는 것과 상관계수를 왝고하는 것은 다르다)
    trimmed = pair.copy()
    for col in (x, y):
    ...
    # 6) 상관 강도 해석 라벨
    # |r| > 0.7 -> 강함, 0.3 < |r| <= 0.7 -> 중간, 0 < |r| <= 0.3 -> 약함, 그외 -> 없음
    a = abs(coef)
    if a > 0.7 :    
        strength = 'Strong'
    elif a > 0.3 :  
    ...
    if plot:
        my_plot.scatterplot(data=pair, x=x, y=y, palette=palette, title=title, xlabel=xlabel, ylabel=ylabel,
...
def multi_correlation(data, columns=None, alpha=0.05, plot=True, palette=None,
                      diag_kind='kde', reg=False, title=None, width=1280, height=1024, save_path=None):
    ...
    # 1) 준비 작업
    # 분석에 사용할 컬럼 결정 (지정하지 않으면 수치형 컬럼 전체 적용)
    if columns is None:
        columns = data.select_dtypes(include='number').columns.tolist()
    ...
    # 4) 산점도 행렬 시각화
    if plot:
        my_plot.pairplot(data=data[columns], palette=palette, diag_kind=diag_kind, reg=reg, title=title, width=width, height=height, save_path=save_path)
...
def compute_vif(df, columns= None):
    '''
    각 변수의 VIF를 statsmodels 패키지로 계산해서 변환    
    '''
    ...
    if columns is None:
        ...
        # VIF 는 수치 행렬 연산이므로 비수치형이 섞이면 안된다.
        non_numeric = list(target.select_dtypes(exclude='number').columns)
        if non_numeric:
            raise TypeError(f'수치형이 아닌 컬럼은 VIF를 계산할 수 없습니다 : {non_numeric}')
    
    # 2) VIF 계산
    # 회귀모형에 절편(상수항)이 있어야 올바른 VIF가 나오므로 절편(상수항)을 추가
    X = add_constant(target)

    # 각 변수(열)별로 VIF를 하나씩 계산해서 리스트에 담는다.
    vif_values= []

    # 각 변수에 대해 VIF를 계산
    for i in range(X.shape[1]):
        # variance_inflation_factor는 i번째 변수에 대한 VIF를 계산
        vif_i = variance_inflation_factor(X.values, i)
        vif_values.append(vif_i)

    # 계산한 VIF 값들을 변수명과 함께 DataFrame으로 정리
    vif = DataFrame({'VIF' : vif_values}, index=X.columns)

    # 상수항(const)은 분석 대상이 아니므로 제외
    # 인덱스 오름차순으로 먼저 정렬
    # 그 후 VIF 기준 내림차순으로 정렬해서 반환
    # 단, 동점일 경우 기존 순서를 유지하도록 kind = 'stable'옵션 사용
    vif = (vif.drop('const').sort_index().sort_values(by='VIF', ascending=False, kind='stable'))

    return vif

#------------------------------
# 적합도 검정 함수 정의
def chi2_goodness_of_fit(data, column, expected=None, order=None, alpha=0.05, plot=True, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    '''
    ...
    # 2) 기대 빈도 결정
    # None 면 균등분포, 합이 1 이하인 비율이면 빈도로 환산, 그 외에는 빈도로 사용
    if expected is None:
        exp = np.full(k, n/k)
    ...
    # 5) 강도 확인
    # 효과 크기 : Cohen's w = sqrt(chi2 / n)
    w = float(np.sqrt(chi2 / n))

    # 효과 크기 강도 라벨
    # --> (Cohen 관례 : 0.1 미만 - 미미 / 0.3 미만 - 약함 / 0.5 미만 - 보통 / 그 이상 강함)
    if w < 0.1 : 
        strength = 'Negligible'
    elif w < 0.3 :
        strength = 'Weak'
...
def _chi2_crosstab(data, row, col, kind, alpha=0.05, plot=True, palette=None, orient='v', 
    ...
    Args : 
        ...
        row (str) : 행 범주형 변수명
        col (str) : 열 범주형 변수명
        kind (str) : 검정 종류 (독립성 검정 : 'independence' / 동질성 검정 : 'homogheneity')
        alpha (float) : 유의수준 (기본값 : 0.05)
        plot (bool) : 결과를 시각화할지 여부 (기본값 : True)
        palette : (str or list) : 색상 팔레트 (기본값 : None)
    ...
    # 효과크기 강도 라벨 (Cohen 관례 : 0.1 미만 - 미미 / 0.3 미만 - 약함 / 0.5미만 - 보통 / 그 이상 강함)
    if cramers_v < 0.1:
        strength = 'Negligible'
    elif cramers_v < 0.3 : 
        strength = 'Weak'
    ...
    if plot : 
        ...
        my_plot.stackplot(data=tmp, x=row, y='_n', hue=col, orient=orient, aggfunc='sum', 
...
def chi2_independence(data, x, y, alpha=0.05, plot=True, palette=None, orient='v',
    ...
    '''
    return _chi2_crosstab(data, x, y, 'independence', alpha, plot=plot, palette=palette, orient=orient, title=title,
                          xlabel=xlabel, ylabel=ylabel, width=width, height=height, save_path=save_path)


# 동질성 검정 함수 확인

def chi2_homogeneity(data, group, category, alpha=0.05, plot=True, palette=None, orient='v',
                      title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    '''
    ...
    '''
    return _chi2_crosstab(data, group, category, 'homogeneity', alpha, plot=plot, palette=palette, orient=orient, title=title,
                          xlabel=xlabel, ylabel=ylabel, width=width, height=height, save_path=save_path)
...
def correlation_summary(corr_df, strength='Strong'):
    ...
    Args : 
        corr_df (DataFrame) : multi-correlation()이 리턴한 변수 쌍별 상관분석 결과표
    ...
    # 1) 결과표를 양방향으로 만들기
    # corr_df 는 각 쌍이 한 번만 등장하므로(A-B만 있고, B-A는 없음)
    ...
    # 2) 변수별 최강 상대
    # 절대값이 큰 순으로 정렬했으므로 그룹별 첫 번째 항목이 곧 최강 상태가 된다.
    # 강한 쌍이 없는 변수도 '중복 없음' 이라는 근거로 남아야하므로 여기서는 강도로 거르지않는다.
