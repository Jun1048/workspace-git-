...
from . import my_stats, my_qtcheck
from . import my_plot
from . import my_prep
...
def fit_model(data, y, summary=False):
    '''
    statsmodels 의 OLS를 이용해 선형회귀 모델을 적합한다.
    ...
    if y not in data.columns:
        raise KeyError (f'종속변수 "{y}"가 데이터프레임의 컬럼에 존재하지않습니다.')
...
#-------------------------------------------
# 선형성 가정 검정 함수 정의
def test_linear(fit, alpha=0.05, plot=True, palette=None, 
    ...
    잡아내지 못한 곡선 관계가 남아있다는 뜻이므로 선형성 가정이 위배된다.
    ...
    Args : 
        ...
        palette (str) : 산점도 점 색상에 사용할 팔레트 이름, None 이면 기본색 (기본값 : None)
        title (str) : 그래프 전체 제목 (기본값 : None)
        xlabel (str, optional) : x축 라벨 (기본값 : None -> '적합값(예측값)')
        ylabel (str, optional) : y축 라벨 (기본값 : None -> '잔차 (resudual)')
    ...
    fvalue = float(reset_res.fvalue)    # F통계량
...
def test_normal(fit, alpha=0.05, plot=True, palette=None, width=1280, height=640):
    ...
    # 1. 잔차추출 및 표본수에 따른 검정 선택
    resid = fit.resid                           # 잔차 (residual) 추출
    n = len(resid)                              # 표본수 (n) 확인

    if n < 30 :                                 # 표본수가 30미만이면, Shapiro-Wilk 검정 사용
        method = 'Shapiro-Wilk'                 # Shapiro-Wilk 검정 통계량 및 p값
        s, p = shapiro(resid)
    else : 
        ...
        # 표준화 방식으로 동일한 검정을 수행)
        mu = resid.mean()                   # 잔차평균
        sigma = resid.std(ddof=1)           # 잔차 표준편차 (표본 분산)
        z = (resid - mu) / sigma            # 잔차 표준화
        s, p = kstest(z, 'norm')            # 표준정규분포와 비교환 K-S 검정 통계량 및 p값

    s = float(s)
    p = float(p)
    normality = bool(p >= alpha)            # 정규성 가정 충족여부 (True, False)
    ...
    if plot:
        ...
        my_plot.lmplot(
            ...
            ylabel = '표본 분위수 (Sample Quantitles)',
    ...
    for k, exp in zip((1, 2, 3), expected):
        ...
        se = np.sqrt(exp * (1 - exp) / n) # 기대 비율의 표준오차
        lo = max(0.0, exp - 2 * se)
        hi = min(1.0, exp + 2 * se)
        ok = bool(lo <= actual <= hi)         

        mse_pass.append(ok)         # 구간별 규칙 충족여부를 리스트에 저장
        mse_rows.append({
            '구간' : f'±{k}√MSE',
            '기대(%)' : round(exp * 100, 1),
...
def test_independent(fit):
    ...
    데이터에서는 독립성이 위배되도라도 무시해도 되는 경우가 많다.
    
    시각화가 필요하지 않은 검정이므로 plot파라미터를 제공하지 않음
...
def report_fitness(fit, log_y = False, log_x = None, log1p_y = False, log1p_x = None,
    ...
    summary() 결과표의 문자열을 파싱하지 않고, 'fit' 객체가 이미 갖고잇는 속성에서
    지표를 직접 읽어와 문장을 구성한다. 표에 보이는 값은 반올림된 표시값이지만
    'fit'은 완전한 정밀도의 실수값을 갖고 있으므로 보고 형식에 맞춰 round()로 자리수만 맞춘다.

    변환이 적용된 변수는 문장에 log(...) / log1p(...)로 표기하여 실제 적합한 모형을 그대로 드러낸다.
    ...
    for x in xnames:
        ...
        if hc3:
            ...
            row = {
                ...
                "β": beta,                  # 표준화 회귀계수(β) = B x (독립변수 표준편차 / 종속변수 표준편차)
...
def report_variables_text(fit, log_y=False, log_x=None, log1p_y=False, log1p_x=None, 
    ...
    # 7) 로버스트 표준오차 log1p 사용 시 해석 주의 각주 첨부
    if hc3:
            report += (
...
def fit_pipeline(data, y, nominal_cols=None, *, 
    ...
    if save_path:
        ...
        if ext in ('.xlsx', '.xls'):
            ...
            raise ValueError(f"{ext}(은)는 지원하지 않는 저장 형식입니다")
...
def compare_models(fits, metric='RMSE', sub_metric='변수수', tolerance=0.05,
    ...
    종속변수를 log1p·반사 변환한 모델은 예측값을 원본 척도로 되돌려 RMSE·MAE·R^2를 계산한다.
